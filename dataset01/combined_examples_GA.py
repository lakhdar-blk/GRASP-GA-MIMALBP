...
import combined_examples as rx1
import numpy as np
...
Ind = np.arange(5*rx1.nb_tasks).reshape(5,rx1.nb_tasks)
...
def crossover_function(array_M):
    ...
    #print(array_M)
    tab = np.arange(rx1.nb_solutions*rx1.nb_tasks).reshape(rx1.nb_solutions, rx1.nb_tasks)
    #tab = array_M
    ...
    t3 = [None]*rx1.nb_tasks
    t4 = [None]*rx1.nb_tasks
    t5 = [None]*rx1.nb_tasks
    t6 = [None]*rx1.nb_tasks
    ...
    for i in range(3, rx1.nb_tasks): t5[i] = t2[i] #copy of the last seven elements from t2 to t5
    for i in range(3, rx1.nb_tasks): t6[i] = t1[i] #copy of the last seven elements from t1 to t6

    for i in range(3):

        for j in range(3, rx1.nb_tasks):
    ...
    for j in range(rx1.nb_solutions):
        t = tab[j]
        for i in range(1,rx1.nb_tasks+1):
            if not i in t :
                for k in range(rx1.nb_tasks):
                    if t[k] == 0 :
                        t[k] = i
                        break
...
def mutation_function(array):
    ...
    for j in tab:
        ...
        #dans chaque élement d'indice a séléctionnés avant mélanger les taches de 2 a 7  
        tm = array[j]
