...
Ind = np.arange(5*md1.nb_tasks).reshape(5,md1.nb_tasks)
...
def crossover_function(array_M):
    ...
    #print(array_M)
    tab = np.arange(md1.nb_solutions*md1.nb_tasks).reshape(md1.nb_solutions, md1.nb_tasks)
    #tab = array_M
    ...
    t3 = [None]*md1.nb_tasks
    t4 = [None]*md1.nb_tasks
    t5 = [None]*md1.nb_tasks
    t6 = [None]*md1.nb_tasks
    ...
    for i in range(3, md1.nb_tasks): t5[i] = t2[i] #copy of the last seven elements from t2 to t5
    for i in range(3, md1.nb_tasks): t6[i] = t1[i] #copy of the last seven elements from t1 to t6

    for i in range(3):

        for j in range(3, md1.nb_tasks):
    ...
    for j in range(md1.nb_solutions):
        t = tab[j]
        for i in range(1,md1.nb_tasks+1):
            if not i in t :
                for k in range(md1.nb_tasks):
                    if t[k] == 0 :
                        t[k] = i
                        break
    
    #print(tab)
    #tab = md1.precedence_relations(tab)
    #print(tab)
...
def mutation_function(array):
    ...
    print(array)        
    """
    array = md1.precedence_relations(array)
...
c_times = md1.cycle_time(pop) #cycle times of first individuals
print(c_times)
print("-------------------------------")
